...
export default function App() {
  ...
  const maxWait = history.length > 0
    ? Math.max(...history.map(h => parseFloat(h.predicted_time)))
    : 0;
  const minWait = history.length > 0
    ? Math.min(...history.map(h => parseFloat(h.predicted_time)))
    : 0;
  ...
  const doughnutData = {
    labels: ['Short (≤10m)', 'Medium (≤30m)', 'Long (≤60m)', 'Very Long'],
    datasets: [{
      data: [
      ...
      {activeTab === 'predict' && (
        <div style={s.mainRow}>
          <div style={s.formCard}>
            ...
            <div style={s.formGrid}>
              ...
              <div>
                ...
                <select value={form.queue_type}
                  onChange={e => setForm({...form, queue_type: e.target.value})}
                  style={s.input}>
            ...
            <button onClick={() => { setForm({people:'', avg_time:'', queue_type:'General', priority:'Normal', counter:'1'}); setResult(null); setError(''); }}
              style={s.clearBtn}>
              🔄 Clear Form
            </button>
          </div>
            ...
            {result ? (
              <>
                <div style={s.resultBox}>
                  ...
                  <div style={{
                    ...s.statusBadge,
                    background: getStatus(result.wait_time).bg,
                    color: getStatus(result.wait_time).color
                  }}>
                ...
                <div style={s.tipBox}>
                  {parseFloat(result.wait_time) <= 10
                    ? '✅ Short wait! You can join the queue now.'
                    : parseFloat(result.wait_time) <= 30
                    ? '⚠️ Moderate wait. Grab a seat nearby.'
                    : parseFloat(result.wait_time) <= 60
                    ? '🔴 Long wait. Consider scheduling an appointment.'
              ...
              <div style={s.emptyResult}>
                ...
                <p style={{color: '#9a3412', fontSize: 16}}>
      ...
      {activeTab === 'history' && (
        <div style={s.card}>
          ...
          <div style={{overflowX: 'auto'}}>
            ...
            {history.length === 0 &&
              <p style={{textAlign:'center', color:'#9a3412', padding: 40}}>
                No records yet!
              </p>
            }
          </div>
          ...
          <div style={{...s.mainRow, gridTemplateColumns: '1fr 2fr'}}>
            ...
            <div style={s.card}>
              ...
              <div style={s.summaryGrid}>
                {[
                  { label: 'Total Queues Processed', val: history.length, icon: '📋' },
                  { label: 'Total People Served', val: totalPeople, icon: '👥' },
                  { label: 'Average Wait Time', val: formatTime(avgWait), icon: '⏱️' },
                  { label: 'Maximum Wait Time', val: formatTime(maxWait), icon: '🔺' },
                  { label: 'Minimum Wait Time', val: formatTime(minWait), icon: '🔻' },
                  { label: 'Short Queues (≤10m)', val: history.filter(h => parseFloat(h.predicted_time) <= 10).length, icon: '✅' },
                  { label: 'Medium Queues (≤30m)', val: history.filter(h => parseFloat(h.predicted_time) > 10 && parseFloat(h.predicted_time) <= 30).length, icon: '⚠️' },
                  { label: 'Long Queues (>30m)', val: history.filter(h => parseFloat(h.predicted_time) > 30).length, icon: '🔴' },
                ].map((item, i) => (
